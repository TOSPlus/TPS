...
OPTIONS = Options()
OPTIONS.search_path = os.environ["PORT_TOOLS"]
OPTIONS.signapk_path = "android/signapk.jar"  # Relative to search_path
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir,
  ...
  if os.path.exists(prebuilt_path):
    ...
    bootable_img = BuildBootableImage(os.path.join(unpack_dir, tree_subdir),
                                         os.path.join(unpack_dir, fs_config),
                                         info_dict)
    if bootable_img is not None:
      return File(name, bootable_img)
    else:
      return None
...
def ParseOptions(argv,
  ...
  os.environ["PATH"] = (OPTIONS.search_path + os.pathsep + os.environ["PATH"])
  os.environ["PATH"] = (os.path.join(OPTIONS.search_path, "android") +
    ...
    for k, v in sorted(current.iteritems()):
      if v:
        ...
        while True:
          ...
          #                            % (k,)).strip()
          # hendysu modified, use a dummy password
          result[k] = "m/EgNsm2iEFLNSGpgEMtyA=="
          if result[k]: break
    return result

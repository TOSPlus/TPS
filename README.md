...
【本周推荐】
新增  护眼功能（较暗环境下亮屏，屏幕亮度平滑变亮）


【桌面】
修复  白色壁纸显示白色文字的问题
修复  应用安装进度条可能概率性不消失的问题
修复  恢复数据过程中一键整理，微信自动下载失败的问题
修复  本地搜索页面的应用列表中不显示未安装完成的应用图标
修复  dock栏上文件夹内应用恢复后图标置灰的问题（N5 5.1机型）
修复  更新/安装应用在桌面上不显示水波纹进度的问题（一加4.3机型）
修复  快捷图标恢复后，显示的图标比正常图标小的问题（一加4.3机型）
修复  下载应用过程中，切换“最美的遇见”主题，提示“桌面停止运行”的问题
修复  商店下载应用并暂停，重启手机后桌面带水波纹的应用图标显示异常的问题（小米3联通版机型）
修复  英文模式下进入云服务，退出到桌面切换成中文后提示“桌面停止运行”的问题（三星S3机型）

【设置】
新增  护眼功能（较暗环境下亮屏，屏幕亮度平滑变亮）

【相册】
修复  部分图片无法显示为128×128的问题
修复  通过便签进入最近照片，图片不停加载的问题（三星Note3机型）
修复  相机拍照后，新建便签添加图片显示最近照片为0的问题（三星S4机型）

【云服务】
修复  点击登录循环跳入登录界面的问题

【时钟】
修复  定时器弹窗提醒时，拨打电话，定时器依然会发出提示音的问题

【便签】
修复  系统设置为最大字体时，便签列表界面内容和时间会重叠的问题
修复  英文模式下，编辑便签的完成按钮显示不完整的问题（N5 4.4机型）
修复  快速地反复进入图片便签详情界面，图片加载失败的问题（三星S4机型）

【省电】
修复  省电界面与锁屏、状态栏显示不一致的问题
